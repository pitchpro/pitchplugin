...
if( !class_exists('PitchPro_Pitch') ){
    class PitchPro_Pitch{
        ...
        function init(){
            ...
            add_filter( 'wp_title', array( $this, 'fix_wp_title' ), 20 );
        }
            ...
            if( !empty($entry['post_id']) ){
                ...
                if( self::POSTTYPE == $pitch->post_type && ( empty($pitch->post_name) || !is_valid_guid($pitch->post_name) ) ){
                    ...
                    foreach( $form['fields'] as $field){
                        print_r($field->inputName);
                        if( $field->inputName == 'pitch_offer_decision' ){
                            // $pitch->post_title = $entry[ $field->id ];
                        }
                    }
        ...
        public function fix_wp_title( $title ){
            global $post;
            if($post->post_type == self::POSTTYPE){
                $title = 'PitchPro | Pitch for ' . get_field('send_to');
            }
            return $title;
        }
